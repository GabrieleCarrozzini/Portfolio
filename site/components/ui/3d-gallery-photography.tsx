...
function GalleryScene({
  ...
  /* Use refs for velocity/autoplay — avoids React re-renders in the animation loop */
  ...
  /* Attach wheel to the Three.js canvas element, keyboard to document */
  useEffect(() => {
  ...
  /* Resume auto-play after 3 s of inactivity */
  useEffect(() => {
  ...
  useFrame((state, delta) => {
    ...
    planesData.current.forEach((plane, i) => {
      ...
      /* Update mesh position directly via the plane data — read during render below */
      void halfRange; // reference to silence TS
    });
  });
...
function FallbackGallery({ images }: { images: ImageItem[] }) {
  ...
  return (
    <div style={{ display: 'flex', flexDirection: 'column', alignItems: 'center', justifyContent: 'center', height: '100%', background: 'var(--black-900)' }}>
      <div style={{ display: 'grid', gridTemplateColumns: 'repeat(3, 1fr)', gap: 8, maxHeight: 400, overflowY: 'auto' }}>
        {normalizedImages.map((img, i) => (
          // eslint-disable-next-line @next/next/no-img-element
          <img key={i} src={img.src} alt={img.alt} style={{ width: '100%', height: 120, objectFit: 'cover' }} />
        ))}
      </div>
    </div>
  );
